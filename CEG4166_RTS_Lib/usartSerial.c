...
/******************************************************************************//*!
 * \mainpage USART Asynchronous Serial Module
 *
 ...
 *
 * Note that printf can be replace by other names (e.g. print to print an unformated string).
 *************************************************************************************************/
...
typedef enum
{
	VACANT,//!< USART is available for access
	ENGAGED//!< USART is engaged by another task.
} USART_STATE;
...
typedef struct
{
	volatile uint8_t *udrPtr;  //!< Pointer to data regiser UDRn
 ...
 * Can be changed using setDefaultUSART()
  *************************************************************************************************/
USART_ID defaultUSART = USART0_ID;   //< default USART - defaults to USART0


//--------------------------------Prototypes Local Functions-----------------------
// Polling functions
inline int8_t usartCheckRxComplete(USART_ID);
inline int8_t usartCheckRxComplete(USART_ID);
inline int8_t usartCheckTxReady(USART_ID);
...
// Interrupt section
inline void xmitInterrupt_On(USART_ID );
inline void xmitInterrupt_Off(USART_ID );
...
USART_ID usartOpen(USART_ID usartId, uint32_t ulWantedBaud, uint16_t uxTxQueueLength, uint16_t uxRxQueueLength )
{
	...
	usartComBuf[usartId].workBufferInUse = VACANT;  // clear the occupation flag.
...
{
	while(usartComBuf[usartId].workBufferInUse == ENGAGED ) _delay_us(25);
	usartComBuf[usartId].workBufferInUse = ENGAGED;
	...
	usartComBuf[usartId].workBufferInUse = VACANT;
}
...
void usart_xfprintf_arg(USART_ID usartId, const char * format, va_list arg)
{
	while(usartComBuf[usartId].workBufferInUse == ENGAGED ) taskYIELD();
	usartComBuf[usartId].workBufferInUse = ENGAGED;
...
// Receive interrupts
ISR(USART0_RX_vect) { usart_rx_isr(USART0_ID); }
ISR(USART1_RX_vect) { usart_rx_isr(USART1_ID); }
ISR(USART2_RX_vect) { usart_rx_isr(USART2_ID); }
ISR(USART3_RX_vect) { usart_rx_isr(USART3_ID); }
...
/*-----------------------------------------------------------*/
// Transmit interrupts
ISR(USART0_UDRE_vect) { usart_tx_isr(USART0_ID); }
ISR(USART1_UDRE_vect) { usart_tx_isr(USART1_ID); }
ISR(USART2_UDRE_vect) { usart_tx_isr(USART2_ID); }
ISR(USART3_UDRE_vect) { usart_tx_isr(USART3_ID); }
