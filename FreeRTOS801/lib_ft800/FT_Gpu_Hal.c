...
//ft_uint8_t FT_DispSwizzle = 3;	// Gameduino2
ft_uint8_t FT_DispSwizzle = 0;		// 4DSystems FT843
...
ft_bool_t  FT_GPU_HAL_Open(FT_GPU_HAL_Context_t *host)
{
	...
	spiSetClockDivider(SPI_CLOCK_DIV4);		// SPI at 1/4 SYSCLK speed (5529kHz on Goldilocks, 4000kHz on Uno)
...
{
	spiSetClockDivider(SPI_CLOCK_DIV2);		// SPI at 1/2 SYSCLK speed (11059kHz on Goldilocks, 8000kHz on Uno)
}
...
inline ft_uint16_t FT_GPU_Cmdfifo_Freespace(FT_GPU_HAL_Context_t *host)
{
