...
#endif
#if _FS_REENTRANT
	SemaphoreHandle_t	sobj;	/* Identifier of sync object */
#endif
#if !_FS_READONLY
...
FRESULT f_mkfs (const TCHAR* path, uint8_t sfd, uint16_t au);				/* Create a file system on the volume */
FRESULT f_fdisk (uint8_t pdrv, const uint32_t szt[], void* work);			/* Divide a physical drive into some partitions */
int16_t f_putc (FIL* fp, TCHAR c);											/* Put a character to the file */
int16_t f_puts (FIL* cp, const TCHAR* str);									/* Put a string to the file */
int16_t f_printf (FIL* fp, const TCHAR* str, ...);							/* Put a formatted string to the file */
TCHAR*  f_gets (FIL* fp, TCHAR* buff, int len);								/* Get a string from the file */
...
/* Sync functions */
#if _FS_REENTRANT
int16_t	ff_cre_syncobj (uint8_t vol, SemaphoreHandle_t* sobj);	/* Create a sync object */
int16_t	ff_req_grant (SemaphoreHandle_t sobj);					/* Lock sync object */
