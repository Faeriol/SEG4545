...
SemaphoreHandle_t xSPISemaphore = NULL; // removed STATIC to allow ramfs.c to use same semaphore.
...
inline void spiSetClockDivider(SPI_CLOCK_DIV_t rate)
{
...
inline void spiSetDataMode(SPI_MODE_t mode)
{
...
inline uint8_t spiSelect(SPI_SLAVE_SELECT SS_pin)	/* 1:Successful, 0:Timeout */
{
...
inline void spiDeselect(SPI_SLAVE_SELECT SS_pin)
{
