...
/                   Improved behaviour of volume auto detection.
...
#endif
#if _MAX_SS == _MIN_SS
#define	SS(fs)	((uint16_t)_MIN_SS)	/* Fixed sector size */
#else
...
#else
#error Number of volumes must be between 1 to 10 inclusive.
#endif
...
#ifdef _EXCVT
static
const uint8_t ExCvt[] = _EXCVT;	/* Upper conversion table for extended characters */
#endif
...
uint32_t get_fattime (void)
{
...
#if defined( portRTC_DEFINED ) // DS1307 I2C chip RTC

	if( getDateTimeDS1307( &y2ktime ) == pdFALSE ) // set to zero if time and date can't be read. Clock error.
		return (uint32_t) 0;
    ...
    return ( (uint32_t)system_fatfs( &y2ktime ) );
}	// get_fattime()
...
int16_t ff_cre_syncobj (	/* TRUE:Function succeeded, FALSE:Could not create due to any error */
	...
	SemaphoreHandle_t *sobj		/* Pointer to return the created sync object */
)
{
	int16_t ret;
...
int16_t ff_del_syncobj (	/* TRUE:Function succeeded, FALSE:Could not delete due to any error */
	SemaphoreHandle_t sobj		/* Sync object tied to the logical drive to be deleted */
)
{
	int16_t ret;
...
int16_t ff_req_grant (	/* TRUE:Got a grant to access the volume, FALSE:Could not get a grant */
	SemaphoreHandle_t sobj	/* Sync object to wait */
)
{
	int16_t ret;

	ret = (xSemaphoreTake(sobj, (_FS_TIMEOUT  / portTICK_PERIOD_MS )) == pdTRUE);	/* FreeRTOS */
...
void ff_rel_grant (
	SemaphoreHandle_t sobj	/* Sync object to be signalled */
)
{
...
FRESULT chk_lock (	/* Check if the file can be accessed */
	...
	int16_t acc			/* Desired access type (0:Read, 1:Write, 2:Delete/Rename) */
)
{
	uint16_t i, be;
...
static
int16_t enq_lock (void)	/* Check if an entry is available for a new object */
{
	uint16_t i;
...
static
uint16_t inc_lock (	/* Increment object open counter and returns its index (0:Internal error) */
...
FRESULT dir_read (
	DIR* dp,		/* Pointer to the directory object */
	int16_t vol		/* Filtered by 0:file/directory or 1:volume label */
)
{
	FRESULT res;
	uint8_t a, c, *dir;
	...
	while (dp->sect) {
		...
		a = dir[DIR_Attr] & AM_MASK;
#if _USE_LFN	/* LFN configuration */
		if (c == DDE || (!_FS_RPATH && c == '.') || (int16_t)((a & ~AM_ARC) == AM_VOL) != vol) {	/* An entry without valid data xxx patch 1 June 6 2014 */
			ord = 0xFF;
		} else {
			if (a == AM_LFN) {			/* An LFN entry is found */
				if (c & LLE) {			/* Is it start of LFN sequence? */
...
#else		/* Non LFN configuration */
		if (c != DDE && (_FS_RPATH || c != '.') && a != AM_LFN && (int16_t)((a & ~AM_ARC) == AM_VOL) == vol)	/* Is it a valid entry?  xxx patch 1 June 6 2014 */
			break;
#endif
		res = dir_next(dp, 0);				/* Next entry */
		if (res != FR_OK) break;
...
)
{
	...
	if ((uint16_t)*path < ' ') {			/* Null path name is the origin directory itself */
		res = dir_sdi(dp, 0);
		dp->dir = 0;
	} else {								/* Follow path */
		for (;;) {
...
static
int8_t get_ldnumber (	/* Returns logical drive number (-1:invalid drive) */
...
)
{
	...
	if (*path) {	/* If the pointer is not a null */
		for (tt = *path; (uint16_t)*tt >= (_USE_LFN ? ' ' : '!') && *tt != ':'; tt++) ;	/* Find ':' in the path */
...
static
uint8_t check_fs (	/* 0:FAT boot sector, 1:Valid boot sector but not FAT, 2:Not a boot sector, 3:Disk error */
	FATFS* fs,	 	/* File system object */
...
)
{
	...
	*rfs = fs;							/* Return pointer to the file system object */
	...
	stat = disk_initialize(fs->drv);	/* Initialise the physical drive */
...
)
{
	...
	if (fp->err)						/* Check error */
        ...
        if(fp->cltbl) { /* Fast seek */
                uint32_t tlen = *fp->cltbl;
                if(ofs == CREATE_LINKMAP) { /* Create CLMT */
                        uint32_t *tbl = fp->cltbl;
                        //uint32_t tlen = *tbl;
                        tbl++;
                        uint32_t ulen = 2; /* Given table size and required table size */
                        uint32_t cl = fp->sclust; /* Top of the chain */
                        if(cl) {
                                do {
                                        /* Get a fragment */
                                        uint32_t tcl = cl;
                        ...
                        *fp->cltbl = ulen; /* Number of items used */
                        ...
                        if(ofs) {
                                ...
                                uint32_t dsc = clust2sect(fp->fs, fp->clust);
...
#endif
#endif
			if (!w || chk_chr("\"*+,.:;<=>\?[]|\x7F", w) || j >= (uint16_t)((w >= 0x100) ? 10 : 11)) /* Reject invalid characters for volume label */
...
FRESULT f_mkfs (
	...
	uint16_t au			/* Allocation unit [bytes] */
)
{
	...
	i = (fmt == FS_FAT32) ? au : (uint16_t)n_dir;
	do {
		if (disk_write(pdrv, tbl, wsect++, 1))
			return FR_DISK_ERR;
	} while (--i);
...
)
{
	...
	if (i >= (int16_t)(sizeof pb->buf) - 3) {	/* Write buffered characters to the file */
		f_write(pb->fp, pb->buf, (uint16_t)i, &bw);
		i = (bw == (uint16_t)i) ? 0 : -1;
	}
	pb->idx = i;
	pb->nchr++;
...
)
{
	...
	if (   pb.idx >= 0	/* Flush buffered characters to the file */
		&& f_write(pb.fp, pb.buf, (uint16_t)pb.idx, &nw) == FR_OK
		&& (uint16_t)pb.idx == nw) return pb.nchr;
	return EOF;
}
...
)
{
	...
	for (;;) {
		...
		case 'S' :					/* String */
			...
			continue;
		case 'C' :					/* Character */
			putc_bfd(&pb, (TCHAR)va_arg(arp, int16_t)); continue;
		case 'B' :					/* Binary */
			r = 2; break;
		case 'O' :					/* Octal */
			r = 8; break;
